...
The build system is set up to compile an executable called `test_zenium`
...
To run the unit tests manually, launch `src/test/test_zenium`. To recompile
...
To run the GUI unit tests manually, launch `src/qt/test/test_zenium-qt`
...
`test_zenium` accepts the command line arguments from the boost framework.
For example, to run just the `getarg_tests` suite of tests:

```bash
test_zenium --log_level=all --run_test=getarg_tests
```
...
```bash
test_zenium --run_test=getarg_tests/doubledash
```
...
Run `test_zenium --help` for the full list of tests.
...
For debugging you can launch the `test_zenium` executable with `gdb` or `lldb` and
start debugging, just like you would with any other program:

```bash
gdb src/test/test_zenium
```

#### Segmentation faults

If you hit a segmentation fault during a test run, you can diagnose where the fault
is happening by running `gdb ./src/test/test_zenium` and then using the `bt` command
within gdb.
...
`test_zenium` arguments and ensure that your ulimits are set properly (e.g. `ulimit -c
unlimited`).
