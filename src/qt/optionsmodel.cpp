...
/** Call node.updateRwSetting() with Zenium 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Zenium 22.x releases try to read these specific settings as
...
//! Convert enabled/size values to zenium -prune setting.
...
//! Get pruning enabled value to show in GUI from zenium -prune setting.
...
//! Get pruning size value to show in GUI from zenium -prune setting. If
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if zenium.conf value
