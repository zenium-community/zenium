When Zenium Core automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no Zenium node is listening,
or one where a service is listening that is not related to the Zenium network.
As a result, this service may occasionally get connection attempts from Zenium
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by Zenium Core,
trying to connect because it thinks there is a Zenium node on that
...
Below is a list of "bad" ports which Zenium Core avoids when choosing a peer to
