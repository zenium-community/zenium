# CJDNS support in Zenium Core

It is possible to run Zenium Core over CJDNS, an encrypted IPv6 network that
...
done outside of Zenium Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Zenium network and individual nodes.
...
before it will work with your Zenium Core node. This is described in steps
...
## Run Zenium Core with CJDNS

Once you are connected to the CJDNS network, the following Zenium Core
...
When enabled, this option tells Zenium Core that it is running in an
...
IPv6 local network. This helps Zenium Core perform better address management:
...
CJDNS support was added to Zenium Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `zenium-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/zenium-community/zenium/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Zenium Core

There are several ways to see your CJDNS address in Zenium Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `zenium-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `zenium-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `zenium-cli help getnodeaddresses` for details.
