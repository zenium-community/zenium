...
On unix systems, the `--enable-multiprocess` build option can be passed to `./configure` to build new `zenium-node`, `zenium-wallet`, and `zenium-gui` executables alongside existing `bitcoind` and `zenium-qt` executables.
