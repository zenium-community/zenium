UNIX BUILD NOTES
====================
Some notes on how to build Zenium Core in Unix.
...
memory available when compiling Zenium Core. On systems with less, gcc can be
...
To build Zenium Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
If you want to build zenium-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a zenium-qt executable will be
built by default.
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, Zenium Core can
