# PSBT Howto for Zenium Core

Since Zenium Core 0.17, an RPC interface exists for Partially Signed Zenium
...
PSBT is an interchange format for Zenium transactions that are not fully signed
...
Overall, the construction of a fully signed Zenium transaction goes through the
following steps:
...
- An **Extractor** produces a valid Zenium transaction (in network format)
...
#### Multisig with multiple Zenium Core instances
...
Zenium Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `zenium-cli` is used.
