# User-space, Statically Defined Tracing (USDT) for Zenium Core

Zenium Core includes statically defined tracepoints to allow for more
...
**Note**: Zenium Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Zenium Core
...
Multiple tools can list the available tracepoints in a `zeniumd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Zenium Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/zeniumd
stap net        outbound_message 0x0000000000107c05 /src/zeniumd
stap validation block_connected  0x00000000002fb10c /src/zeniumd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Zenium Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/zeniumd | grep NT_STAPSDT -A 4 -B 2
