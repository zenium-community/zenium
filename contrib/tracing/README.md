...
Tracing (USDT) support for Zenium Core on Linux using. For more information on
USDT support in Zenium Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `zeniumd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `zeniumd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Zenium Core USDT documentation] on how to list available tracepoints in your
`zeniumd` binary.

[Zenium Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/zeniumd
```
...
Zenium P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/zeniumd
```
...
In a different terminal, starting Zenium Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/zeniumd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/zeniumd
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/zeniumd
```
