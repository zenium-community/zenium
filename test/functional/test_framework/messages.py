...
"""Zenium test framework primitive and message structures
...
# Objects that map to zeniumd objects, which can be serialized/deserialized
...
class CBlockLocator:
    ...
    def serialize(self):
        r = b""
        r += (0).to_bytes(4, "little", signed=True)  # Zenium Core ignores the version field. Set it to 0.
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in zeniumd
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class msg_version:
    ...
    def deserialize(self, f):
        ...
        # But, unconditionally check it to match behaviour in zeniumd
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in zeniumd indicates these should be deserialized as blocks
