...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave zeniumds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop zeniumds after the test execution")
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: zeniumds were not stopped and may still be running")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a zeniumd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple zeniumd test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the zeniumd
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as zeniumd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in zenium.conf
    ...
    def skip_if_no_bitcoind_tracepoints(self):
        """Skip the running test if zeniumd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("zeniumd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if zeniumd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("zeniumd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if zenium-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("zenium-wallet has not been compiled")

    def skip_if_no_bitcoin_util(self):
        """Skip the running test if zenium-util has not been compiled."""
        if not self.is_bitcoin_util_compiled():
            raise SkipTest("zenium-util has not been compiled")
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether zenium-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bitcoin_util_compiled(self):
        """Checks whether zenium-util was compiled."""
